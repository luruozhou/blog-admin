...
var _self: {
    _thenfailCaptureStartLine?: Function,
    _thenfailCaptureEndLine?: Function
} = {};
...
(<any>_self)._thenfailCaptureStartLine = () => ThenFail.Utils.captureLine();
...
class ThenFail<T> implements ThenFail.Thenable<T> {
    ...
    static mapLimit<T, R>(items: T[], limit: number, handler: (item: T, index: number) => ThenFail.Thenable<R> | R): ThenFail<R[]> {
        var mapped: R[] = [];
        ...
        for (let i = 0; i < items.length; i++) {
            let j = Math.floor(i / limit);

            if (!blocks[j]) {
                blocks[j] = [];
            }

            blocks[j].push(items[i]);
        }
        ...
        return ThenFail
            .each(blocks, block => {
                return ThenFail
                    .map(block, handler)
                    .then(blockMapped => {
                        mapped = mapped.concat(blockMapped);
                    });
            })
            .then(() => mapped);
    }
...
module ThenFail {
    ...
    export class PromiseLock {
        ...
        ready<T>(handler?: () => Thenable<T> | T): ThenFail<T> | ThenFail<void> {
    ...
    export module Utils {
        ...
        export function captureBoundaries(): void {
            thenfailStartLine = (<any>_self)._thenfailCaptureStartLine();
            thenfailEndLine = (<any>_self)._thenfailCaptureEndLine();

            delete (<any>_self)._thenfailCaptureStartLine;
            delete (<any>_self)._thenfailCaptureEndLine;
        }
    }
}

(<any>_self)._thenfailCaptureEndLine = () => ThenFail.Utils.captureLine();
